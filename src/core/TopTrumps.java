...
public class TopTrumps {
	...
	public static void setUpGame(int noOfPlayers, String playerName) {
		...
		// resets the round counter to 0 for each new game
		roundCounter = 0;
		...
		// each writeLog function, writes this information to our log file throughout
		// the game
		writeLog(cards);
		...
		// if -c is selected then will go straight into round after setting up the game
		if (commandLineMode) {
			roundStage1();
		}
	}
	...
	public static void roundStage1() {
		...
		// step 1 - selects top card from each player
	...
	public static void roundStage2() {
		...
		for (int i = 0; i < players.length; i++) {
			if (!eliminated[i]) {
				s += "\n Player " + (i + 1) + " value is: "
						+ topCards.getCards().get(i).getCharacteristics()[chosenAttribute].getValue();
			}
		}
		writeLog(s);
...
//		int roundWinner = -1;
		...
		for (int i = 0; i < topCards.getCards().size(); i++) {
			if (!eliminated[i]) {
				if (topCards.getCards().get(i).getCharacteristics()[chosenAttribute]
						.getValue() > topCards.getCards().get(getRoundWinner()).getCharacteristics()[chosenAttribute]
								.getValue()) {
					setRoundWinner(i);
				}
			}
		...
		// step 4 - shows the winner and their card and lets user know they will pick
		...
		// step 5 - if draw: transfer all cards to the communal pile
		// winner: transfer all cards from round to back of winners cards
		// also if winner and there is cards in the com pile, add them to back and empty
		// com pile
		if (draw) {
		...
		// step 6 - if winner the game will end and show stats of that game but not
		// coded yet and offer main menu to player
		// will also check if any player has 0 cards and eliminate them
		// then offer human player to proceed to next round (not coded yet also) -
		// commented below what should happen
		...
		if (gameEnded) {
			...
			// if chosen to exit game database will disconnect and the system will close
			if (choice == 0) {
				DatabaseLogic.disconnectDatabase();
				System.exit(0);
				// if chosen to return to menu, this will display the main menu
			} else if (choice == 1) {
