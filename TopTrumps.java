...
public class TopTrumps {
	...
	public static void round(int playerChooseAttribute, Deck communalPile) {
		...
		for(int i = 0; i<players.length; i++) {
			if(!eliminated[i]) {
				System.out.println(players[i].getName() + " " + players[i].getDeck().getCards().size());
				System.out.println("have cards: \n" + players[i].getDeck().toString());
...
//				topCards.addCard(players[i].getDeck().removeCard(players[i].getDeck().getCards().size() - 1));
		...
		System.out.println("The cards in play: \n" + topCards.toString() + "That's all");
		System.out.println("\n\n----------------round: " + roundCounter + "----------------");
		if(!eliminated[0]) {		
			System.out.println("----------------Your Card on Top is:----------------\n" + topCards.getCards().get(0));
		}
